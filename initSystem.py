...
def checkout():
    ...
    if not (Path(modules_path).is_dir() and Path(datasetFolder).is_dir() and Path(gpt_path).is_dir() and Path(race_path).is_dir() and Path(squad_path).is_dir() and Path(stsb_path).is_dir() and Path(datasetFilterFolder).is_dir()):
